"""
Modified code from https://github.com/tensorflow/tensorflow/blob/master/tensorflow/examples/speech_commands/models.py
We tried to exactly implement model from 'Convolutional Neural Networks for Small-footprint Keyword Spotting' paper

- one_fstride4
- trad_fpool3

Our implemented models have same # of flops as 'Hello Edge: Keyword Spotting on Microcontrollers' paper reported
"""
...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
  ...
  elif model_architecture == 'one_fstride4':
    return create_one_fstride4_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'trad_fpool3':
    return create_trad_fpool3_model(fingerprint_input, model_settings, is_training)
    ...
    raise Exception('model_architecture argument "' + model_architecture +
                    ...
                    ' "low_latency_conv, "one_fstride4", "trad_fpool3", "low_latency_svdf"' +
                    ' or "tiny_conv"' +
                    '')
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  if is_training:
    first_dropout = tf.nn.dropout(first_relu, 0.5)
  else:
  ...
  if is_training:
    second_dropout = tf.nn.dropout(second_relu, 0.5)
  else:
...
def create_trad_fpool3_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.

  This is exactly the network labeled as 'cnn-trad-fpool3' in the
  'Convolutional Neural Networks for Small-footprint Keyword Spotting' paper:
  http://www.isca-speech.org/archive/interspeech_2015/papers/i15_1478.pdf
  ...
  fingerprint_4d = tf.reshape(fingerprint_input,
                              [-1, input_time_size, input_frequency_size, 1])
  first_filter_width = 8
  first_filter_height = 20  # input_time_size // 3 * 2  # spans almost 2/3
  first_filter_count = 64
  first_weights = tf.get_variable(
      name='first_weights',
      initializer=tf.truncated_normal_initializer(stddev=0.01),
  ...
  max_pool = tf.nn.max_pool(first_dropout,
                            [1, p_pooling_in_time, q_pooling_in_frequency, 1],
                            [1, p_pooling_in_time, q_pooling_in_frequency, 1],
                            'VALID')
  second_filter_width = 4
  second_filter_height = 10
  ...
  # low-rank linear layer (lin)
  first_linear_output_channels = 32
  first_linear_weights = tf.get_variable(
      name='linear_weights',
      initializer=tf.truncated_normal_initializer(stddev=0.01),
      shape=[second_conv_element_count, first_linear_output_channels])
  first_linear = tf.matmul(flattened_second_conv, first_linear_weights)

  first_fc_output_channels = 128
  first_fc_weights = tf.get_variable(
      name='first_fc_weights',
      initializer=tf.truncated_normal_initializer(stddev=0.01),
      shape=[first_linear_output_channels, first_fc_output_channels])
  first_fc = tf.matmul(first_linear, first_fc_weights)
...
def create_low_latency_conv_model(fingerprint_input, model_settings,
  ...
  if is_training:
    second_fc_input = tf.nn.dropout(first_fc, 0.5)
  else:
  ...
  if is_training:
    final_fc_input = tf.nn.dropout(second_fc, 0.5)
  else:
...
def create_one_fstride4_model(fingerprint_input, model_settings, is_training):
  """Builds a convolutional model with low compute requirements.
  This is exactly the network labeled as 'cnn-one-fstride4' in the
  'Convolutional Neural Networks for Small-footprint Keyword Spotting' paper:
  http://www.isca-speech.org/archive/interspeech_2015/papers/i15_1478.pdf
  ...
  # low-rank linear layer (lin)
  first_linear_output_channels = 32
  first_linear_weights = tf.get_variable(name="first_linear_weights",
                                         initializer=tf.truncated_normal_initializer(stddev=0.01),
                                         shape=[first_conv_element_count, first_linear_output_channels])
  first_linear = tf.matmul(flattened_first_conv, first_linear_weights)
  ...
  second_fc_weights = tf.get_variable(
      ...
      initializer=tf.truncated_normal_initializer(stddev=0.01),
      shape=[first_fc_output_channels, second_fc_output_channels])
  second_fc = tf.matmul(second_fc_input, second_fc_weights)
  if is_training:
    final_fc_input = tf.nn.dropout(second_fc, 0.5)
  else:
    final_fc_input = second_fc
